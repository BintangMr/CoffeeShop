...
@section('title','Liberica Admin | Video')

@section('title.page','Video')

@section('page.Video','active')
        ...
        <div class="col-12">
            <div class="card-box">
                ...
                <div class="row col-12 mb-2 px-0">
                    <h4 class="mt-0 header-title col-12 col-md-8 mb-2">Gallery Video</h4>
                    @include('admin.gallery.video.create')
                </div>

                <table id="table_video" class="table table-striped table-bordered dt-responsive nowrap">
                    <thead>
                    <tr>
                        <th>#</th>
                        <th>Preview</th>
...
@push('js')
    ...
    <script>
        function deleteVideo(id, text = null) {
            Swal.fire({
                title: 'Apa anda yakin?',
                text: `Anda akan menghapus video ${text} !`,
                icon: 'warning',
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type: 'DELETE',
                        url: '{{ route('admin.gallery.video.destroy','') }}/' + id,
                        success: function (data) {
                            Swal.fire({
                                title: "Berhasil!",
                                text: `Video ${text} berhasil di hapus!`,
                                type: "success",
                            }).then(() => {
                                window.table_video.ajax.reload(null, false);
                            })
                        },
        ...
        $(document).ready(function () {
            window.table_video = $('#table_video').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.gallery.video.index') }}",
                order: [[2, "asc"]],
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            return `<button class="btn btn-warning waves-effect waves-light" onclick="editVideo('${data}')">
                                        <i class="fa fa-pencil-alt mr-1"></i>
                                        <span>Edit</span>
                                    </button>
                                    <button class="btn btn-danger waves-effect waves-light" onclick="deleteVideo('${data}','${row.url}')">
