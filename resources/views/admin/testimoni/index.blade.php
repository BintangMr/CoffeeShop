...
@section('title','Liberica Admin | Testimoni')

@section('title.page','Testimoni')

@section('page.Testimoni','active')
        ...
        <div class="col-12">
            <div class="card-box">
                ...
                <div class="row col-12 mb-2 px-0">
                    <h4 class="mt-0 header-title col-12 col-md-8 mb-2">Testimoni</h4>
                    @include('admin.Testimoni.create')
                </div>

                <table id="table_testimoni" class="table table-striped table-bordered dt-responsive nowrap">
                    <thead>
                    <tr>
                        <th>#</th>
                        <th>Nama</th>
...
@push('js')
    ...
    <script>
        function deleteTestimoni(id, text = null) {
            Swal.fire({
                title: 'Apa anda yakin?',
                text: `Anda akan menghapus testimoni ${text} !`,
                icon: 'warning',
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type: 'DELETE',
                        url: '{{ route('admin.testimoni.destroy','') }}/' + id,
                        success: function (data) {
                            Swal.fire({
                                title: "Berhasil!",
                                text: `Testimoni ${text} berhasil di hapus!`,
                                type: "success",
                            }).then(() => {
        ...
        $(document).ready(function () {
            window.table_testimoni = $('#table_testimoni').DataTable({
                processing: true,
                serverSide: true,
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            return `<button class="btn btn-warning waves-effect waves-light" onclick="editTestimoni('${data}')">
                                        <i class="fa fa-pencil-alt mr-1"></i>
                                        <span>Edit</span>
                                    </button>
                                    <button class="btn btn-danger waves-effect waves-light" onclick="deleteTestimoni('${data}','${row.nama}')">
