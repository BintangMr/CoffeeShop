...
@section('title','Liberica Admin | Serbuk')

@section('title.page','Serbuk')

@section('page.product.serbuk','active')

@section('content')
    <div class="row">
        @include('admin.product.serbuk.category.index')

        <div class="col-12">
            <div class="card-box">

                <div class="row col-12 mb-2 px-0">
                    <h4 class="mt-0 header-title col-12 col-md-8 mb-2">Serbuk</h4>
                    @include('admin.product.serbuk.create')
                </div>

                <table id="table-serbuk" class="table table-striped table-bordered dt-responsive nowrap">
                    <thead>
                    <tr>
                        <th>#</th>
                        <th>Gambar</th>
    ...
    @include('admin.product.serbuk.show')
    @include('admin.product.serbuk.edit')
@endsection
...
@push('js')
    ...
    <script>
        function deleteSerbuk(id, text = null){
            Swal.fire({
                title: 'Apa anda yakin?',
                text: `Anda akan menghapus serbuk ${text} !`,
                icon: 'warning',
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type: 'DELETE',
                        url: '{{ route('admin.product.serbuk.destroy','') }}/'+id,
                        success: function (data) {
                            Swal.fire({
                                title: "Berhasil!",
                                text: `Serbuk ${text} berhasil di hapus!`,
                                type: "success",
                            }).then(() => {
        ...
        $(document).ready(function () {
            window.table_serbuk = $('#table-serbuk').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.product.serbuk.index') }}",
                order: [[ 2, "asc" ]],
                columns: [
                    {data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false},
                    {
                        ...
                        render: function (data) {
                            return data ? `<img src="${data}" alt="Gambar-Serbuk" class="img-thumbnail">` : null;
                        }
                    },
                    {data: 'name', name: 'name'},
                    {data: 'category_serbuk.name', name: 'category_serbuk.name'},
                    ...
                    },
                    {
                        ...
                        width: "20%",
                        render : function (data, type, row) {
                            return `<button class="btn btn-info waves-effect waves-light" onclick="showSerbuk('${data}')">
                                        <i class="fa fa-eye mr-1"></i>
                                        <span>Lihat</span>
                                    </button>
                                    <button class="btn btn-warning waves-effect waves-light" onclick="editSerbuk('${data}')">
                                        <i class="fa fa-pencil-alt mr-1"></i>
                                        <span>Edit</span>
                                    </button>
                                    <button class="btn btn-danger waves-effect waves-light" onclick="deleteSerbuk('${data}','${row.name}')">
