...
@section('title','Liberica Admin | Artikel')

@section('title.page','Artikel')

@section('page.article','active')
        ...
        <div class="col-12">
            <div class="card-box">
                ...
                <div class="row col-12 mb-2 px-0">
                    <h4 class="mt-0 header-title col-12 col-md-8 mb-2">Artikel</h4>
                    @include('admin.article.create')
                </div>

                <table id="table-article" class="table table-striped table-bordered dt-responsive nowrap">
                    <thead>
                    <tr>
                        <th>#</th>
                        <th>Gambar</th>
...
@push('js')
    ...
    <script>
        function deleteArticle(id, text = null) {
            Swal.fire({
                title: 'Apa anda yakin?',
                text: `Anda akan menghapus article ${text} !`,
                icon: 'warning',
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            Swal.fire({
                                title: "Berhasil!",
                                text: `Article ${text} berhasil di hapus!`,
                                type: "success",
                            }).then(() => {
        ...
        $(document).ready(function () {
            window.table_article = $('#table-article').DataTable({
                processing: true,
                serverSide: true,
                ...
                columns: [
                    {data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false},
                    {
                        ...
                        render: function (data) {
                            return data ? `<img src="${data}" alt="Gambar-Article" class="img-thumbnail">` : null;
                        }
                    },
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            return `<button class="btn btn-info waves-effect waves-light" onclick="showArticle('${data}')">
                                        <i class="fa fa-eye mr-1"></i>
                                        <span>Lihat</span>
                                    </button>
                                    <button class="btn btn-warning waves-effect waves-light" onclick="editArticle('${data}')">
                                        <i class="fa fa-pencil-alt mr-1"></i>
                                        <span>Edit</span>
                                    </button>
                                    <button class="btn btn-danger waves-effect waves-light" onclick="deleteArticle('${data}','${row.title}')">
